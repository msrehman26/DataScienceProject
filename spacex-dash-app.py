...
app.layout = html.Div(children=[
    ...
    # TASK 4: Scatter chart (payload vs. success with booster version)
...
# TASK 4: Callback for scatter plot based on selected site and payload range
@app.callback(
...
def get_scatter_plot(entered_site, payload_range):
    ...
    # If a specific site is selected, further filter the data by site
    if entered_site != 'ALL':
        filtered_df = filtered_df[filtered_df['Launch Site'] == entered_site]
    
    # Create the scatter plot with Payload on the x-axis, Launch Outcome on the y-axis
    fig = px.scatter(filtered_df, 
                     x='Payload Mass (kg)', 
                     y='class', 
                     color='Booster Version Category',
                     title=f'Payload Mass vs. Launch Outcome for {entered_site if entered_site != "ALL" else "All Sites"}',
